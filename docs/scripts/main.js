...
function _toConsumableArray$1(arr) { if (Array.isArray(arr)) {
...
function btn(el) {
    ...
    isMobile &&
        on(body, 'click', function (_) { return body.classList.contains('close') && toggle(); });
}
...
var _createClass$1 = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) {
    var descriptor = props[i];
    descriptor.enumerable = descriptor.enumerable || false;
    descriptor.configurable = true;
    if ("value" in descriptor) {
...
function corner(data, cornerExternalLinkTarge) {
    ...
    return ("<a href=\"" + data + "\" target=\"" + cornerExternalLinkTarge + "\" class=\"github-corner\" aria-label=\"View source on Github\">" +
...
function main(config) {
    ...
    var aside = '<button class="sidebar-toggle" aria-label="Menu">' +
...
function cover() {
    ...
    return ("<section class=\"cover show\" style=\"background: " + bgc + "\">" +
        '<div class="mask"></div>' +
...
function replace(m, $1) {
    return ('<img class="emoji" src="https://github.githubassets.com/images/icons/emoji/' +
        $1 +
        '.png" alt="' +
        $1 +
        '" />');
...
var linkCompiler = function (ref) {
    ...
    return (renderer.link = function (href, title, text) {
        ...
        if (!isAbsolutePath(href) &&
            ...
            attrs.push(href.indexOf('mailto:') === 0
                ? ''
                : linkRel !== ''
                    ? (" rel=\"" + linkRel + "\"")
                    : '');
                    ...
                    if (token.embed.type === 'markdown') {
                        ...
                        text = text.replace(/\[([^[\]]+)\]\(([^)]+)\)/g, function (x) {
                            ...
                            if (x.substring(linkBeginIndex).startsWith('(.')) {
                                return (x.substring(0, linkBeginIndex) +
...
function loadNested(path, qs, file, next, vm, first) {
    ...
    get(vm.router.getFile(path + file) + qs, false, vm.config.requestHeaders).then(next, function (_) { return loadNested(path, qs, file, next, vm); });
}
...
function fetchMixin(proto) {
    ...
    proto._fetch = function (cb) {
        ...
        // Load nav
        loadNavbar &&
            loadNested(path, qs, loadNavbar, function (text) { return this$1._renderNav(text); }, this, true);
    };
    proto._fetchCover = function () {
        ...
        if (coverpage) {
            ...
            if (path) {
                ...
                get(path + stringifyQuery(query, ['id']), false, requestHeaders).then(function (text) { return this$1._renderCover(text, coverOnly); });
            }
            else {
    ...
    proto.$fetch = function (cb, $resetEvents) {
        var this$1 = this;
        if (cb === void 0)
            cb = noop;
        if ($resetEvents === void 0)
    ...
    proto._fetch404 = function (path, qs, cb) {
        ...
        if (notFoundPage) {
            ...
            request(this.router.getFile(path404), true, requestHeaders).then(function (text, opt) { return this$1._renderMain(text, opt, fnLoadSideAndNav); }, function () { return this$1._renderMain(null, {}, fnLoadSideAndNav); });
            return true;
        }
...
async function themeColor(dsConfig) {
    await delay(300);
    const big_fab = await document.getElementsByClassName('big-fab');
    const share_button__back = await document.getElementsByClassName('share-button__back');
    const share_button__front = await document.getElementsByClassName('share-button__front');
    if (dsConfig.options.theme != undefined) {
        if (dsConfig.options.theme == 'open-window') {
            const change_color = await document.getElementsByClassName('animated');
            ...
            const change_background = await document.getElementsByClassName('btn-fab');
...
const install = (hook, vm) => {
    ...
    const config = Object.assign({}, vm.config.share);
